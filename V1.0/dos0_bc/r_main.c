...
/*------------------------------------*/
static void interrupt far oldThandler(void);
static void interrupt far oldThandler()
{
  _disable();
...
static void interrupt far newThandler(void);
static void interrupt far newThandler()
{
  _disable();
  ms_ticks++;
  if(ms_ticks >999)
...
#define     DOS_TMR0      0x0040
#define     DOS_TMRCON    0x0043
static  void SetVector(void);
static  void SetVector()
{
  ...
  _SetEXRegByte(DOS_TMR0, 0xa9);  /* ;Set the timer count (LO byte).*/
  _SetEXRegByte(DOS_TMR0, 0x04);  /* ;Set the timer count (HI byte). */

  num=0x1c;
  (long)ptr = num*4;
  *ptr=(long)((void far *)newThandler);
}
extern	int	sys_pno;
/*================================================================*/
static	int	last_ms;
